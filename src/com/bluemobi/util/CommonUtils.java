...
import com.bluemobi.util.imgUtil.ImagePropUtil;
...
//		String regex = "^0{0,1}(13[0-9]|15[7-9]|153|156|18[7-9])[0-9]{8}$";
...
//        LOGGER.info("读取文件【{}】结束...", new Object[]{fileName} );
     ...
     * 根据数据最大条数和每页条数计算最大页数
     */
    public static int getPageCount(long count,int pageNum){
    	int pageCount = (int) (count/pageNum);
		int mod = (int) (count%pageNum);
		if(mod > 0){
			pageCount++;
		}
		return pageCount;
     ...
     * 根据分页对象获得接口返回MAP
     */
    public static Map<String, Object> getResultmapByPage(Page page){
    	
    	Map<String, Object> resultMap = new HashMap<String, Object>();
    	//计算最大页数
    	int pageCount = CommonUtils.getPageCount(page.getCount(), page.getPageSize());
    	if(page.getData()!=null){
    		resultMap.put("dataList", page.getData());//返回数据集合
    	}else{
    		resultMap.put("dataList", new ArrayList<Object>());//返回数据集合
    	}
    	resultMap.put("count", page.getCount());//返回最大条数
    	resultMap.put("pageCount", pageCount);//返回最大页数
    	resultMap.put("currentPage", page.getPage());//当前页
    	return resultMap;
    }
     ...
     * 获取图片服务器地址
     */
    public static String getImgerverPath(){
    	return ImagePropUtil.getValue("ftp_visi_path");
    }
     ...
     * 根据元图地址返回压缩图片地址
     */
    public static String getMinImgerverPath(String imgPath){
    	if(imgPath == null || imgPath.equals("") || (imgPath.indexOf(".") == -1)){
    		return "";
    	}
    	int i=imgPath.lastIndexOf(".");
    	if(imgPath.substring(i).equals(".jpg")){
    		return imgPath.substring(0,i)+"_min.JPEG";
    	}else{
    		return imgPath.substring(0,i)+"_min"+imgPath.substring(i);
    	}
